...
#define RIGHT A5              // Right IR sensor connected to analog pin A2 of Arduino Uno:
...
#define ECHO_PIN A4           // Echo pin connected to analog pin A0 of Arduino Uno:
...
if((distance > 5) && (distance < 20)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.
...
}else if(distance <5) {                          //If the condition is 'true' then the statement below will execute:
...
}else if(distance <3) {                          //If the condition is 'true' then the statement below will execute:
  
  //Stop
  Motor1.setSpeed(50);  //define motor1 speed:
  Motor1.run(BACKWARD);   //rotate motor1 clockwise:
  Motor2.setSpeed(50);  //define motor2 speed:
  Motor2.run(BACKWARD);   //rotate motor2 clockwise:
  Motor3.setSpeed(50);  //define motor3 speed:
  Motor3.run(BACKWARD);   //rotate motor3 clockwise:
  Motor4.setSpeed(50);  //define motor4 speed:
  Motor4.run(BACKWARD);
}
}
